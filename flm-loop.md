...
| backbone · block | base | **fly adapter** | direct-input control | constant-feature control | relabeled wiring | fly + TTT (best setting) |
|---|---|---|---|---|---|---|
| GPT-2 base (124M) · FLM-equivalent (`c = 0`, K = 1) | 3.6668 | **3.5989** | 3.5985 | 3.6433 | 3.6837 | 3.5978 |
| GPT-2 base · looped, unsigned (`c = 0.8`, `h = 0.7`, K ≤ 8) | 3.6668 | **3.5987** | 3.5985 | 3.6433 | 3.6762 | 3.5977 |
| GPT-2 base · looped + NT signs (gain 0.95) | 3.6668 | **3.5982** | 3.5985 | 3.6433 | 3.6600 | 3.5970 |
| GPT-2 medium (355M) · FLM-equivalent (`c = 0`, K = 1) | 3.2506 | **3.1916** | 3.1910 | 3.2325 | 3.2492 | 3.1911 |
| GPT-2 medium · looped, unsigned | 3.2506 | **3.1912** | 3.1910 | 3.2325 | 3.2385 | 3.1909 |

*Perplexities, first row: 39.1 → 36.6 / 36.5 / 38.2 / 39.8. The constant-feature control is the same adapter fed an all-ones feature, so it can only learn one fixed logit offset — a corpus prior; it depends on the backbone, not on the block, which is why it repeats within a backbone. TTT: fast weights on the adapter's output projection, learned on each window's first 128 tokens, swept over learning rates 0.05–50 and norm bounds 0.5–5; the best setting is always learning rate 5 with a fast-weight norm of 0.7–0.9 ([`posthoc.json`](flm-loop/results/gpt2-base/flm/posthoc.json) in each run folder; [`scripts/summarize_runs.py`](flm-loop/scripts/summarize_runs.py) regenerates the table).*

### 6.3 Reading

- **FLM's null result replicates in every configuration.** Across two backbones and three blocks the fly adapter and the parameter-matched direct-input control differ by between −0.0003 and +0.0006 nats — five ties. The relabeled-wiring control is worse than the base model in every row (least so for the signed loop), as in FLM: an adapter fitted to one node labeling misreads another.
- **What the adapters learn is a token-conditional corpus prior, not connectome computation.** The constant-feature control captures 0.024 of the 0.068-nat gain on GPT-2 base and 0.018 of 0.059 on GPT-2 medium (a fixed shift toward Gutenberg's token statistics); the direct-input control — a function of the current token only — captures all of it; the fly features add nothing on top, whether the block averages once, iterates to a fixed point, or carries transmitter signs. This is the backbone-free §5 result seen from the language model's side: the block's only asset was ~1.5 tokens of linear memory, and GPT-2 already carries 1,024 tokens of it.
- **The gain shrinks with the backbone**, 0.068 → 0.059 nats from 124M to 355M parameters, in the direction the mechanism predicts (a stronger model needs the corpus prior less) — modestly, because that prior is a property of the text, not of the model's memory. Extrapolating to FLM's 1.2B instruction-tuned backbone and its conversation corpus (a distribution the backbone was largely trained on) gives a gain of order 0.02–0.05 nats for fly and direct alike — consistent with the paper's "slightly better direct control".
- **Test-time training on FLM's readout has no leverage.** With the adapter capped at `0.03·tanh(·)` and the logit correction at RMS 0.25, fast weights learned on a window's first half move the second half's NLL by −0.001 nats in every row (learning rate 5, fast-weight norm 0.7–0.9); at learning rate 50 they hurt. Loop 2 of the [self-improvement notes](runtime-self-improvement.md) needs a substrate with more leverage than a bounded readout — a fast logit bias or a last-layer delta on the backbone itself — which is outside FLM's contract and left for the next study.
...
- **On real pretrained backbones it reproduces FLM's null result and explains it** (§6): with GPT-2 base and medium fetched from a public GCS bucket, the fly adapter and the direct-input control tie in all five block-and-backbone configurations (within 0.0006 nats), a constant-feature control shows the shared gain is a corpus prior that shrinks with backbone size, and test-time training on FLM's bounded readout has no leverage. FLM's own 1.2B backbone and conversation corpus remain the laptop experiment (§6.4).
